...
function useSlider(parameters) {
  ...
  const touchId = React5.useRef();
  const [active, setActive] = React5.useState(-1);
  const [open, setOpen] = React5.useState(-1);
  const [dragging, setDragging] = React5.useState(false);
  const moveCount = React5.useRef(0);
  ...
  const [focusedThumbIndex, setFocusedThumbIndex] = React5.useState(-1);
  const sliderRef = React5.useRef();
  ...
  const previousIndex = React5.useRef();
  ...
  const stopListening = React5.useCallback(() => {
    const doc = ownerDocument(sliderRef.current);
...
function useTransitionTrigger(requestEnter) {
  const [state, dispatch] = React43.useReducer(transitionStateReducer, {
    elementExited: !requestEnter,
    inProgress: false
  });
  const hasTransition = React43.useRef(false);
  const handleEntering = React43.useCallback(() => {
    dispatch("entering");
  }, []);
  const handleEntered = React43.useCallback(() => {
    dispatch("entered");
  }, []);
  const handleExiting = React43.useCallback(() => {
    dispatch("exiting");
  }, []);
...
var _excluded25 = ["children", "className", "lastTransitionedPropertyOnEnter", "lastTransitionedPropertyOnExit", "enterClassName", "exitClassName"];
function CssTransition(props) {
  ...
  const [isEntering, setIsEntering] = React67.useState(!requestedEnter);
  React67.useEffect(() => {
    if (requestedEnter) {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-E6PBCGTE.js.map
